...
allowed-tools: discover_devices, control_device
---
# All Devices On
...
**CRITICAL: You MUST turn on devices sequentially — one device per tool call. Do NOT send multiple control_device calls at once. After each control_device call, wait for its response, then tell the user which device was just turned on, pause for 5 seconds, and only then proceed to the next device.**
...
1. Call `control_device` for ONE device only (emit a tool_use block, do not just describe it)
...
- **NEVER call control_device more than once in a single response. Only one device at a time.**
